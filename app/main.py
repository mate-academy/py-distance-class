...
class Distance:
    ...
    def __add__(self, other: Type[Distance]) -> Type[Distance]:
        if isinstance(other, Distance):
            return Distance(self.km + other.km)
        else:
            return Distance(self.km + other)

    def __iadd__(self, other: Type[Distance]) -> Type[Distance]:
        if isinstance(other, Distance):
            self.km += other.km
        else:
            self.km += other
    ...
    def __mul__(self, factor: float) -> Type[Distance]:
        return Distance(self.km * factor)

    def __truediv__(self, divisor: float) -> Type[Distance]:
